...
// @Summary revokeService
// @Description revokeService
// @Accept  json
// @Produce  json
// @Param   ns     path    string     true        "namespace"
// @Param   svcname     path    string     true        "service name"
// @Success 200 "ok"
// @Router /revokeService/{ns}/{svcname} [get]
...
func (rs *resourceService) revokeService(ic *versioned.Clientset, dc dynamic.Interface, ns string, svcname string) string { // because gateway is exposed as nodePort service
	...
	/*
		...
		for i := range vsList.Items {
			...
			for j := range vs.Spec.Http {
				//log.Printf("[%d] Service Name: %+v, Uri: %+v\n", j, vs.Spec.Http[j].Route[0].Destination.Host, vs.Spec.Http[j].Match[0].Uri)
				host := vs.Spec.Http[j].Route[0].Destination.Host
				uriPrefix := vs.Spec.Http[j].Match[0].Uri.GetPrefix()
				uriRewrite := vs.Spec.Http[j].Rewrite
				port := vs.Spec.Http[j].Route[0].Destination.Port.Number
				log.Printf("[%d] host: %+v, uriPrefix: %+v, uriRewrite: %+v, port: %+v\n", j, host, uriPrefix, uriRewrite, port)
				if uriPrefix == "/"+svcname+"-asg/" {
					continue
				} else {
					list = AddJsonPatch(
						&JSONPatch{
...
// @Summary manageTraffic
// @Description manageTraffic defines circuit breaker policy that applies to traffic intended for a service after routing has occurred.
...
// @Param   svcname     path    string     true        "service name"
// @Success 200 "ok"
// @Router /manageTraffic/{ns}/{svcname} [get]
func (tc *resourceController) _manageTraffic(c *gin.Context) {
	ns := c.Param("ns")
	svcname := c.Param("svcname")
	manageTraffic(ic, ns, svcname)
	c.IndentedJSON(http.StatusOK, gin.H{"msg": "ok"})
}
func manageTraffic(ic *versioned.Clientset, ns string, svcname string) {
...
// TODO send request route agent deployed in cluster
// http://localhost:8080/invokeServiceEndpoint?method=get&path=/api/hello&svcname=detail&endpointurl=http://123.456.7.8/api/list
// https://qiita.com/taizo/items/c397dbfed7215969b0a5
// HTTPリクエストのレスポンスを構造体定義なしでjsonに吐き出す方法
// https://shiimanblog.com/engineering/output-json/
...
// @title SGME service governance API
// @version 1.0
...
func main() {
	...
	*/
	...
	router.GET("/manageTraffic/:ns/:svcname", rc._manageTraffic)
	router.GET("/revokeService/:ns/:svcname", rc._revokeService)
